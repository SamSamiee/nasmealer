...
router.post(
   ...
   async (req, res, next) => {
      ...
      try {
         ...
         const rows = ingredientResults.flatMap(
            (result) => result.rows
         );
         const ingredients = [];
         ...
         rows.forEach(
            ({
               ...
               if (!ingredient) {
                  ...
                  if (
                     !ingredient.used_in.includes(meal_name)
                  ) {
...
router.post("/", authenticate, async (req, res, next) => {
   ...
   try {
      ...
      const {
         ...
         "INSERT INTO products (name, created_by) VALUES ($1, $2) ON CONFLICT (name, created_by) DO UPDATE SET name = EXCLUDED.name RETURNING id",
         [name, userId]
      );
      ...
      res.status(201).json({
         status: "success",
         message: "product inserted in to cart.",
      });
   } catch (err) {
      if (client) {
